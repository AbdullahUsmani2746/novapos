...
const ReportsPage = () => {
  return (
    <div className="min-h-screen">
      {/* Header Section */}
      <div className="bg-white  shadow-lg border-gray-200/60 backdrop-blur-sm">
        <div className="container mx-auto px-6 py-8">
          <div className="flex flex-col space-y-4">
            ...
            {/* Stats Bar */}
            <div className="flex items-center space-x-6 ml-9 pt-4">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {Object.entries(REPORT_CONFIG).map(([key, report]) => (
            <Card
              key={key}
              className="group relative overflow-hidden border-0 shadow-md hover:shadow-xl transition-all duration-300 bg-white/80 backdrop-blur-sm hover:bg-white hover:-translate-y-1 flex flex-col"
            >
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-secondary from-secondary/90 via-transparent to-purple-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      {report.status && (
                        <Badge
                          variant="secondary"
                          className={`mt-2 text-xs font-medium ${getStatusColor(
                            report.status
                          )}`}
                        >
              ...
              {/* Hover Border Effect */}
              <div className="absolute inset-0 rounded-lg border-2 border-secondary opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
            </Card>
          ))}
        </div>
        ...
        {Object.keys(REPORT_CONFIG).length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-6">
              Reports will appear here once they are configured.
            </p>
            <Button>
              <TrendingUp className="h-4 w-4 mr-2" />
