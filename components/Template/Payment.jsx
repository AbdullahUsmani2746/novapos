...
export const generatePaymentPDF = () => {
  ...
  const voucherTitle = "PAYMENT VOUCHER";
  ...
  const entries = [
    {
      acctNo: "1127",
      accountTitle: "AMAR ASSOCIATES.-MNT",
      amount: "1,500,000.00",
    },
    {
      acctNo: "1127",
  ...
  const totalAmount = "3,137,363.00";
  const amountInWordsText =
    "Rupees Three Million One Hundred Thirty-Seven Thousand Three Hundred Sixty-Three Only.";
  ...
  const drawHeader = (yPosition = margin) => {
    ...
    doc.text(headerValues.trNo, margin + labelW + gap + 2, row2Y + 4);

    // Check#
    const checkX = margin + labelW + gap + valueW + gap + 30;
    doc.setFillColor(245);
    doc.rect(checkX, row2Y, labelW, rowHeight, "FD");
    doc.text("Check#", checkX + 2, row2Y + 4);

    doc.rect(checkX + labelW + gap, row2Y, 25, rowHeight); // empty box

    // User
    const userX = checkX + labelW + gap + 25 + gap;
    doc.setFillColor(245);
  ...
  const drawFooter = (yPosition) => {
    ...
    const preparedX = margin + 20;
    doc.line(preparedX, lineY, preparedX + lineWidth, lineY);
    doc.text("Prepared by", preparedX + lineWidth / 2, signatureY, {
      align: "center",
    });

    const approvedX = pageWidth / 2 - lineWidth / 2;
    doc.line(approvedX, lineY, approvedX + lineWidth, lineY);
    doc.text("Approved by", approvedX + lineWidth / 2, signatureY, {
      align: "center",
    });

    const receivedX = pageWidth - margin - 20 - lineWidth;
    doc.line(receivedX, lineY, receivedX + lineWidth, lineY);
    doc.text("Received by", receivedX + lineWidth / 2, signatureY, {
      align: "center",
    });
  };
